...
public class NoticeController {
	...
	public String removeNotice(HttpServletRequest request, int noticeNo) {
		...
		return "redirect:/admin/noticeAdminList";
	}
	...
	//[윤경환] 관리자 공지사항  리스트
	@GetMapping("/admin/noticeAdminList")
	public String getNoticeAdminList(Model model, 
			@RequestParam(defaultValue = "1") int currentPage, 
			@RequestParam(defaultValue = "전체") String noticeCategory) {
			log.debug("[Debug] \"START\" noticeController.noticeList() | Get");
			log.debug(" ├[param] currentPage : "+currentPage);
			...
			log.debug(" ├[param] pageNo : " + "pageNo");
			model.addAttribute("pageNo", pageNo);
		return "admin/noticeAdminList";
	}
	//[윤경환] 관리자가 볼수 있는 공지 사항 세부 정보
	@GetMapping("/admin/noticeAdminOne")
	public String getNoticeAdminOne(int noticeNo, Model model) {
		Notice notice = noticeService.getNoticeOne(noticeNo);
		
		model.addAttribute(notice);
		
		return "admin/noticeAdminOne";
	}
	
	//[윤경환]notice 수정
	@GetMapping("/admin/modifyAdminNotice")
	public String modifyNotice(Model model, int noticeNo) {
		...
		return "admin/modifyAdminNotice";
	}
	@PostMapping("/admin/modifyAdminNotice")
	public String modifyNotice(Notice notice) {
		...
		return "redirect:/admin/noticeAdminList?noticeNo="+notice.getNoticeNo();
	}
